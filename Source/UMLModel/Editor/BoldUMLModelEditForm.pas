...
type
  TBoldModelEditFrm = class(TForm, IUnknown, IUMLModelPlugInContext)
  ...
  private
    ...
    procedure SetPlugInList(PlugInList: TList);
...
procedure TBoldModelEditFrm.SetPlugInList(PlugInList: TList);
var
  i: Integer;
begin
  FPlugInList := PlugInList;
  for i := 0 to PlugInList.Count - 1 do
    InstallPlugIn(TObject(PlugInList.Items[i]) as TUMLPlugIn);
end;
...
begin
  ...
  begin
    ...
    CursorGuard := TBoldCursorGuard.Create;
    try
      UMLLinkClass := BoldUMLModelLinkList.LinkClasses[-1, ExtractFileExt(odOpenModel.FileName)];
      if Assigned(UMLLinkClass) then
      begin
        with UMLLinkClass.create(nil) do
        try
          FileName := odOpenModel.FileName;
...
begin
  ...
  begin
    ...
    if not TBoldUMLOperationSupport.ClassHasOperation(UMLClass, GetMethodName(FrameworkMethods[Index]), TypeList) then
    begin
      MenuItem := TMenuItem.Create(popTree);
      MenuItem.Caption := GetMethodName(FrameworkMethods[Index]);
...
begin
  ...
  begin
    ...
    NewMethod.name := GetMethodName(Method);
...
begin
  ...
  for Index := 0 to Length(FrameworkMethods) - 1 do
  begin
    if GetMethodName(FrameworkMethods[Index]) = MethodName then
    begin
...
begin
  TModelEdit.SaveFormsettingsToRegistry(self);
