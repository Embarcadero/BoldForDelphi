...
TBACurrency* __fastcall TProduct::_Get_M_retail_Price()
{
  // assert(ValidateMember("TProduct", "retail_Price", 2, __classid(TBACurrency)));
  TBACurrency *result = dynamic_cast<TBACurrency*>(BoldMembers[2]);
  return result;
};

Currency __fastcall TProduct::_Getretail_Price()
{
  Currency result = M_retail_Price->AsCurrency;
  return result;
}
...
TBoldGeneratedCodeDescriptor *CodeDescriptor = GeneratedCodes()->AddGeneratedCodeDescriptorWithFunc("DerivedAttrExampleClasses", &InstallBusinessClasses, &InstallObjectListClasses, GeneratedCodeCRC());
